...
exports.DispatchService = {
    async dispatch(requestId, pickupLat, pickupLng, emergencyTypeName) {
        // Determine required equipment level
        let requiredEquipmentLevel = 1;
        if (emergencyTypeName) {
            const et = await database_1.prisma.emergencyType.findUnique({ where: { name: emergencyTypeName } });
            if (et)
                requiredEquipmentLevel = et.requiredEquipmentLevel;
        }
        ...
        if (ambulances.length === 0) {
            throw new AppError_1.AppError('No ambulances available at this time', 503);
        }
        // Score and rank ambulances based on distance and capabilities
        const ranked = ambulances
            .map((amb) => {
            const distanceKm = (0, geo_1.haversineDistance)(amb.lat, amb.lng, pickupLat, pickupLng);
            const etaSeconds = (0, geo_1.estimateTravelTime)(distanceKm);
        ...
        if (!targetHospitalId) {
            ...
            const allHospitals = await database_1.prisma.hospital.findMany();
            if (allHospitals.length > 0) {
                const nearestHosp = allHospitals
                    .map(h => ({ id: h.id, dist: (0, geo_1.haversineDistance)(pickupLat, pickupLng, h.lat, h.lng) }))
        ...
        logger_1.logger.info(`Dispatched ambulance ${nearest.ambulance.plateNumber} to request ${requestId} ` +
        ...
        if (nearest.ambulance.driverId) {
            notifPromises.push(database_1.prisma.notification.create({
                data: {
